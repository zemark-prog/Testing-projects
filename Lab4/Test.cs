...
namespace Lab4
{
	...
	public class TestFileWorker
	{
		...
		public int? GetID()
		{
			return my.GetIntBySql("SELECT MAX(FileID) FROM Files");
		}
		...
		{
			bool deleted = my.ExecSql("DELETE FROM Files");
			//clearing db
			Assert.True(deleted);
			string path = @"/Users/markzegelman/Mark.txt";
			string name = BaseFileWorker.GetFileName(path);
			...
			byte[] textInBytes = Encoding.UTF8.GetBytes(text);
			bool added = my.AddFile(name, textInBytes);
			bool added2 = my.AddFile(name, textInBytes);
			int counter = 2;
			Assert.True(added);
			Assert.True(added2);
			System.Data.DataTable res = my.GetFiles(name);
			Assert.IsNotNull(res);
			Assert.AreEqual(counter, res.Rows.Count);
		}
	...
	public class TestBinaryFlag
	{
		...
		public int? GetID()
		{
			return my.GetIntBySql("SELECT MAX(MultipleBinaryFlagID) FROM MultipleBinaryFlags");
		}

		[Test()]
		public void TestDifferentInitialization()
		{
			string flagStr = "mark";
